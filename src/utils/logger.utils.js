...
const Logger = {
    getLogContent (prefix, title, desc, arg) {
        arg = arg || {};
        switch (prefix) {
            case 'debug': return `--debug ${title} - 描述：${desc} - 参数：`;
            case 'info': return `--info ${title} - 描述：${desc} - 参数：`;
            case 'warning' : return `--warning ${title} - 修改方案：${desc} - 参数：`;
            case 'error': return `--error ${title} - 解决方案：${desc} - 参数：`;
        }
    },
