<template>
    <div class="ivz-edit-table ivz-border-radius">
        <a-table ref="tableRef" :columns="tableMetas" :data-source="dataSource" bordered
            ...
            <template v-for="col in slotMetas" :slot="col.field" slot-scope="text, row, index">
                <slot :name="col.field" :row="row" :index="index" :meta="col">
                    <div v-if="row['EditableFlag'] && col['editable']" :key="col.field">
                        ...
                        <a-cascader v-else-if="col.type=='cascade'" :options="col.data" :loadData="loadData"
                            :value="row[col.field]" :default-value="col.default" @change="changeHandle(col, row, text)"
                            changeOnSelect :placeholder="col.placeholder"></a-cascader>
                        ...
                        <a-date-picker v-else-if="col.type=='date'" size="small" :value="row[col.field]"
                           :format="col.config.format" :show-time="col.showTime" :ranges="col.ranges" :disabled="disabledHandle(col, row)"
                           @change="(val)=>changeHandle(col, row, val)" :default-value="col.default"
                           :disabled-date="col.disabledDate" :disabled-time="col.disabledTime">
                        </a-date-picker>
                        ...
                        <a-input v-else :type="col.type" :placeholder="col.placeholder"
                             ...
                             :prefix="col.prefix" :suffix="col.suffix" :blur="()=>eventHandle(col, row, 'blur')"
...
<script>
    ...
    export default {
        ...
        created () {
            this.$page.resolverMetas(this.tableMetas, this, (item) => {
                if (item['editable']) {
                    this.slotMetas.push(item)
                    this.$set(this.oriModel, item.field, item.default)
                    this.$set(item, 'scopedSlots', {customRender: item.field})
                    if (!item['formatter']) this.$set(item, 'formatter', this.formatter)
                }
            })
        },
        mounted () {
        ...
        methods: {
            ...
            actionHandle (mate, row, index) {
                let disabled = mate['disabled']
                if (disabled && disabled(row)) return

                if (this.actionMetas.Add === mate) {
                    if (this.isNotBlank(this.backEditModel)) {
                        return this.$msg.warningMessage('请先(保存/取消)当前新增行')
                    }
                    row = this.getOriModel()
                } else if (this.actionMetas.Save === mate) {
                    if (!row['EditableFlag']) return
                } else if (this.actionMetas.Edit === mate) {
                    if (this.isNotBlank(this.backEditModel)) {
                ...
                let url = mate.url
                mate.callBack(row).then(param => {
                    let resolve = this.$utils.getPromiseResolve(param)
                    if (this.actionMetas.Save === mate) {
                        let saveMate = row[this.tableConfig.rowKey] ? this.actionMetas.Edit : this.actionMetas.Add
                        if (!saveMate.url) {
                            return this.$log.warningLog('没有指定提交地址', '请在功能元数据里面设置Url', saveMate)
                        }
                        this.loading = true
                    ...
                    } else if (this.actionMetas.Del === mate) {
                        if (row[this.tableConfig.rowKey]) {
                            this.loading = true
                            let delSubmit = [row[this.tableConfig.rowKey]]
                            this.$http.post(url, delSubmit).then(resp => {
                    ...
                    } else if (this.actionMetas.Add === mate) {
                        this.backEditModel = row // 备份新增对象, 用来判断是否一次新增多条
                        this.dataSource.splice(index, 0, row)
                    } else if (this.actionMetas.Edit === mate) {
                        this.$utils.assignProperty(this.backEditModel, row) // 备份当前编辑底下
                        this.$set(row, 'EditableFlag', true) // EditableFlag说明可编辑
                    } else if (this.actionMetas.Cancel === mate) {
                        let rowKey = this.tableConfig.rowKey
                        ...
                        this.$http.post(url, row).then(resp => {
