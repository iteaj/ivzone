                    ...
                    <a-button-group>
                        <template v-if="searchMainMetas">
                            <a-button v-for="item in searchMainMetas" :key="item.id"
                                      :type="item.color" @click="actionEvent(item.id)">
...
<script>
    export default {
        ...
        methods: {
            actionEvent(id) {
                switch (id) {
                    case 'add': this.$router.push("/IvzSys/add"); break;
            ...
            initActionMates (actionMetas) {
                ...
                Object.keys(actionMetas).forEach(key => {
                    ...
                    this.$page.registerPosition('search', actionMeta, this.searchMainMetas, this.searchMoreMetas)
                })
            }
        }
    }
